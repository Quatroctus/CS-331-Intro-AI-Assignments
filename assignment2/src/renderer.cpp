...
Ref<VertexArray> VertexArray::CreateVertexArray(const VertexLayout& layout, const Ref<VertexBuffer> buffer) {
...
Ref<Font> Font::CreateFont(const std::string& filepath, unsigned char startChar, uint32_t length) {
	assert(255 - startChar + 1 >= length);
	Ref<Font> font = Ref<Font>(new Font(startChar, length));
...
void VertexArray::addVertexBuffer(const VertexLayout& layout, const Ref<VertexBuffer> vertices) {
	glBindVertexArray(this->id);
	vertices->bind();
	uint32_t index = 0;
