...
void SetupGLFWEventCallbacks(GLFWwindow* window) {
    ...
    glfwSetCursorPosCallback(window, [](GLFWwindow* w, double x, double y) {Othello::Mouse::MoveCallback(x, y); });
    glfwSetMouseButtonCallback(window, [](GLFWwindow* w, int button, int pressed, int mods) { if (button == GLFW_MOUSE_BUTTON_1) Othello::Mouse::ButtonCallback(pressed); });
}
...
int main(int argc, char** args) {
    ...
    if (!Othello::ObtainPlayers(args)) return 2;
